        ...
        try {
            ...
            if (status == 'down') {
                ...
                const [insert] = await DB.execute(
                    "INSERT INTO `his_master_trip` (`alat`, `ip`, `tanggal_down`, `waktu_down`) VALUES (?,?,?,?)",
                    [alat,ip,tanggal,time] 
                );
                const [update] = await DB.execute(
                    "UPDATE `real_master_trip` SET `status` = ? where `ip` = ?",
                    [status,ip]
                );
                ...
                if(down_time_key && tanggal_down){
                    ...
                    if(monthDifference === 0){
                        ...
                        const [hai] = await DB.execute(
                            "UPDATE `his_master_trip` SET `tanggal_up` = ?, `waktu_up`=?, `durasi_down`=? where `ip`=? and `tanggal_down`=? and `waktu_down`=?",
                            [tanggal, time, durasidown,  ip, tanggal_down, down_time_key]
                        );
                        const [hai1] = await DB.execute(
                            "UPDATE `real_master_trip` set `status`=? where `ip`=?",
                            [status,ip]
                        );
                        res.status(201).json({
                            ...
                            status: 201,
                            message: "Post has been created successfully",
                            post_id: hai.insertId,
                            post_id: hai1.insertId,
                        });
                        console.log("berhasil insert data ke database");
                        } 
                else {
                    ...
                    const [tableupdate1] = await DB.execute(
                        "UPDATE `real_master_trip` SET `link_status` = ? WHERE `ip` = ?",
                        [status, ip]
                    );
                    results.push({
                        operation: "update1",
                    ...
                    for (var i = 1; i <= monthDifference; i++) {
                        ...
                        if (i === 1) {
                            const [result1] = await DB.execute(
                                "UPDATE `his_master_trip` SET  `Tanggal_Up`=?, `Waktu_Up`=?  WHERE `Tanggal_Down`=? AND `Waktu_Down`=?  and `ip` = ?",
                        ...
                        const [insertupdate] = await DB.execute(
                            "INSERT INTO `his_master_trip` (`alat`, `ip`, `Tanggal_Down`, `Waktu_Down`, `Tanggal_Up`, `Waktu_Up`) VALUES (?,?,?,?,?,?)",
